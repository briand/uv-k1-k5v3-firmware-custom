...
#ifdef ENABLE_CW_MODULATOR
    // Fox Hunt owns the transmitter and reads the keypad itself, so release the side
    // key / paddle inputs from the CW keyer for the duration - otherwise a CW VFO would
    // let SIDE1 or the paddle key up mid-hunt. RADIO_SelectVfos() on the way out puts
    // the keyer back exactly as the current modulation wants it.
